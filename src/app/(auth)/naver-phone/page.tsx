...
import styles from './naver-phone.module.css'

// 네이버 전화번호 정규화: 010-1234-5678 or +82-10-1234-5678 → 01012345678
function normalizeNaverPhone(raw: string | undefined): string {
  if (!raw) return ''
...
export default function NaverPhonePage() {
  ...
  const [hasNaverPhone, setHasNaverPhone] = useState(false)
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)

  useEffect(() => {
    const supabase = createClient()
    supabase.auth.getUser().then(({ data: { user } }) => {
      const naverPhone = user?.user_metadata?.phone_number
      const normalized = normalizeNaverPhone(naverPhone)
      if (normalized) {
        setPhone(normalized)
        setHasNaverPhone(true)
      }
    })
      ...
      <div className={styles.inner}>
        ...
        <div className={styles.field}>
          ...
          <input
            ...
            onChange={hasNaverPhone ? undefined : (e) => {
            ...
            readOnly={hasNaverPhone}
            placeholder="010-0000-0000"
            className={`${styles.input} ${hasNaverPhone ? styles.input_readonly : ''}`}
            maxLength={13}
          />
        </div>
